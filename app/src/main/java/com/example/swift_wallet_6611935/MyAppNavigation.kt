...
import androidx.navigation.compose.composable
import androidx.navigation.compose.rememberNavController
import com.example.swift_wallet_6611935.View.*
import com.example.swift_wallet_6611935.ViewModel.AuthViewModel
...
fun MyAppNavigation(modifier: Modifier = Modifier, authViewModel: AuthViewModel) {
    ...
    NavHost(navController = navController, startDestination = "login") {
        composable("login") { LoginContent(modifier, navController, authViewModel) }
        composable("signup") { RegisterContent(modifier, navController, authViewModel) }
        composable("home") { MainContent(modifier, navController, authViewModel) }
//        composable("UserManual") { UserManualPage(modifier, navController, authViewModel) }
        composable("history") { HistoryScreen(navController, paddingValues = PaddingValues()) }
        composable("transactionPage") { TransactionPage(navController, authViewModel) }
        composable("profile") { ProfileScreen(paddingValues = PaddingValues(),navController, authViewModel) }  // Pass ThemeViewModel
    }
}
